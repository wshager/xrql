...
import module namespace rql="http://lagua.nl/rql" at "../../xrql/rql.xqm";
...
let $maxLimit := 100
let $store := "/db/benjamins.com/en_us/model/" || $model
...
return
    if($method = ("PUT","POST")) then
        ...
        if($id != "") then
            ...
            element {"json:root"} {
                for $x in rql:sequence(collection($store)/json:root,$q,$maxLimit) return
                    element {"json:value"} {
                        attribute {"json:array"} {"true"},
                         $x/node()
                    }
